!function($){
	...
	$.fn.paging=function(op){
		$(this).each(function(){
			...
			//alert('___cur ' + op.current + '___max ' + op.max + '___length ' + op.length + '___end ' + op.end + '___last ' + op.last);
			...
			if(op.current<op.max){	
				...
				//if(op.last!==false && op.end<op.max    op.current-op.end<op.length) make(op,op.max,op.sideClass,op.last);//last button
				if(op.end!=op.max) make(op,op.max,op.sideClass,op.last);//last button
			}
			this.__JQ_PAGING=op;
		});
	};
